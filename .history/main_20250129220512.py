...
def archivist_decide(user_input, conversation_history):
    ...
    try:
        ...
        print("Archivist Failed to Categorize Input")
        return None  
...
def main():
    ...
    while True:
        ...
        # Ask The Archivist to categorize the input
        ...
        if "what do you know about me" in user_input.lower():
            all_data = memory.load_memory()
            if all_data:
                details = []
                for category, values in all_data.items():
                    for key, value in values.items():
                        details.append(f"{key}: {value}")
                print(f"Bot: Here's what I know about you: {', '.join(details)}")
            else:
